...
export class SbbRadioButton extends LitElement {
  ...
  public set checked(value) {
    ...
    this.dispatchEvent(
      new CustomEvent<RadioButtonStateChange>("state-change", {
        bubbles: true,
        detail: { type: "checked", checked: this._checked },
      })
    );
  ...
  public set disabled(value) {
    ...
    this.dispatchEvent(
      new CustomEvent("state-change", {
        bubbles: true,
        detail: { type: "disabled", disabled: this._disabled },
      })
    );
  }
  ...
  private _handlerRepository = new HandlerRepository(
    ...
    namedSlotChangeHandlerAspect(
      (m) => (this._namedSlots = m(this._namedSlots))
    ),
    formElementHandlerAspect
  );
  ...
  private _setupInitialStateAndAttributeObserver(): void {
    ...
    this._radioButtonAttributeObserver.observe(
      this._element,
      radioButtonObserverConfig
    );
  }

  /** Observe changes on data attributes and set the appropriate values. */
  private _onRadioButtonAttributesChange(
    mutationsList: MutationRecord[]
  ): void {
    for (const mutation of mutationsList) {
      if (mutation.attributeName === "data-group-disabled") {
        this._disabledFromGroup = !!isValidAttribute(
          this._element,
          "data-group-disabled"
        );
      }
  ...
  override render() {
    ...
    this.setAttribute(
      "aria-required",
      (this.required || this._requiredFromGroup).toString()
    );
    this.setAttribute(
      "aria-disabled",
      (this.disabled || this._disabledFromGroup).toString()
    );
    setAttribute(
      this,
    ...
    return html`
      <label class="sbb-radio-button">
        ...
        />
        <!-- OR value=${this.value ??
        nothing} both checks only for undefined & null -->
