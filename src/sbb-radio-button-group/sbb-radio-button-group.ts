...
export class SbbRadioButtonGroup extends LitElement {
   ...
   */
  @property({ reflect: true })
  public orientation: InterfaceSbbRadioButtonGroupAttributes["orientation"] =
    "horizontal";
  ...
  public override connectedCallback(): void {
    ...
    this._hasSelectionPanel = !!this._element.querySelector(
      "sbb-selection-panel"
    );
    toggleDatasetEntry(
      this._element,
      "hasSelectionPanel",
      this._hasSelectionPanel
    );

    this.addEventListener(
      "state-change",
      (ev: CustomEvent<RadioButtonStateChange>) => this.onRadioButtonSelect(ev),
      { passive: true }
    );
  ...
  private _setFocusableRadio(): void {
    const checked = this._radioButtons.find(
  ...
  private _getRadioTabIndex(radio: SbbRadioButton): number {
    return (radio.checked || this._hasSelectionPanel) &&
      !radio.disabled &&
      !this.disabled
      ? 0
      : -1;
  ...
  public handleKeyDown(evt: KeyboardEvent): void {
    ...
    if (
      ...
      ((evt.target as HTMLElement) !== this._element &&
        ...
        (evt.target as HTMLElement).parentElement.nodeName !==
          "SBB-SELECTION-PANEL")
    ) {
      return;
    }
    ...
    if (this._hasSelectionPanel) {
      current = enabledRadios.findIndex(
        (e: SbbRadioButton) => e === evt.target
      );
